...
class DDPGActorNetwork:
    ...
    def __init__(self, state_dim, action_dim, layer_sizes, optimizer_type, optimizer_args={}, tau=TAU, conv_layer_info_list=[],
        ...
        inputs = layers.Input(shape=(self.state_dim,)) if type(self.state_dim) == int else layers.Input(shape=self.state_dim)
        ...
        if len(conv_layer_info_list) > 1:
            for conv_layer_info in conv_layer_info_list:
                ...
                out = tf.keras.layers.Conv2D(filters=num_filters, kernel_size=kernel_size, strides=strides, padding=padding, activation='relu')(out)
                if add_pooling:
                    pool_size = conv_layer_info['pool_size'] if 'pool_size' in conv_layer_info else (2, 2)
                    out = tf.keras.layers.MaxPool2D(pool_size=pool_size, strides=conv_layer_info['strides'] if 'strides' in conv_layer_info else None)(out)
